// ===============================
// КОНТЕНТ-СКРИПТ (content)
// ===============================
// Этот файл исполняется на каждой странице (см. manifest.json).
// Здесь создаётся плавающее меню с группами и промтами, а также кнопка‑триггер.
// Контент‑скрипт общается с фоновым скриптом через chrome.runtime.sendMessage,
// чтобы получать/изменять данные.
//
// Основные блоки:
// - создание UI (кнопка, контейнер меню, стили)
// - управление видимостью и позицией меню
// - рендер данных групп/промтов
// - обработчики действий пользователя (копирование, добавление, удаление)
// - реакция на сообщение "refreshMenu" для обновления UI после изменений данных
//
// Весь код старается работать без внешних зависимостей и не конфликтовать со стилями страницы.
// Для этого используются изолированные контейнеры и инлайновые стили.
// src/content.js
import { CONFIG } from './config.js';

// Инициализируем расширение на всех страницах: создаём кнопку активации меню
const triggerButton = createTriggerButton();
document.body.appendChild(triggerButton);

// Создаем контейнер контекстного меню (скрыт по умолчанию)
...
// Стили для header меню и внутренних кнопок
...
// Обработчики событий: клик по триггеру открывает/закрывает меню,
// кнопка крестика закрывает меню
triggerButton.addEventListener('click', toggleMenu);
document.getElementById('plm-close-btn').addEventListener('click', closeMenu);

// Функции управления меню (открыть/закрыть/позиционировать)
function toggleMenu() {
...
function openMenu() {
  ...
  // Загружаем данные у фонового скрипта и рендерим меню
...
function renderMenuContent(data) {
  ...
  // Назначаем обработчики на динамически отрисованные элементы
...
function deletePromptHandler(e) {
  ...
  // Кастомное подтверждение удаления вместо window.confirm,
  // чтобы не ломать стили сайта и держать единый внешний вид
